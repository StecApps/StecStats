...
**How to apply:** Never call a second video `getUserMedia` while the shared iOS recorder is active. Source WebRTC frames from the recording session, keep async live media and peer work session-fenced, and preserve score-only mode as the explicit failure/legacy fallback. Keep the dormant native module graph aligned with the last physically working binary while its feature flag is off; excluding a still-imported native facade coincided with pre-render termination. Require physical cold-entry, repeated-entry, permission, background/foreground, and sustained recording tests before enabling or changing a shared-session binary; keep the score-only fallback available through OTA.