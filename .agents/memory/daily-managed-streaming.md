...
**How to apply:** Persist the live-session-to-game association in the same durable game-save operation, including offline retries. Keep broadcaster signaling authenticated and session-bound. Delay public scores/stats through a durable DB queue to match YouTube latency, and keep YouTube start/finalization retryable and fenced. Use renewable token-fenced leases for long recording imports.