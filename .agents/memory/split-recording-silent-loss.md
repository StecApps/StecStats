...
**Why:** a real user game (game 158) lost its entire second half with no warning — the user only discovered it when the film room "broke" on second-half plays. Silent `resolve(null)` + `if (blob)` skip was the failure mode.
