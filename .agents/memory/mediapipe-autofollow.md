...
- **Viewport gate (added 2026-07-12):** `updateTracker` accepts an optional `TrackViewport { cx, cy, zoom }` matching the currently-rendered canvas crop. When zoom > 1.2, any candidate whose raw-video centre falls outside `[cx±0.5/zoom, cy±0.5/zoom]` is rejected before distance/colour scoring — a coach or spectator who is physically off-screen simply cannot steal the lock. Record.tsx passes `{ trackCenterXRef, trackCenterYRef, trackZoomRef }` every tick. This is the primary fix for the "coach walking by grabbed the lock" failure mode.
- **Size-scaled radius (added 2026-07-12):** radius = `min(TRACK_RADIUS_MAX, max(normHeight*1.5, TRACK_RADIUS_BASE*0.5) + missCount*growth)` instead of a fixed base. When the player is tiny in the wide-court raw frame (normHeight ~0.03–0.05), the acceptance window shrinks proportionally instead of staying at 10% of the full frame width — which was wide enough to reach multiple nearby players.
