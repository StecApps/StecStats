...
/**
 * Send a "game highlights ready" push notification to the game owner — but
 * only if the notification has not already been sent for the current reel
 * (`highlightNotificationSent` is false/null). Marks the flag true after
 * sending so repeated calls are idempotent.
 *
 * Exported for unit testing. Called by generateHighlight after the reel has
 * been successfully written to storage.
 */
export async function maybeSendGameHighlightNotification(
  game: Pick<
    typeof gamesTable.$inferSelect,
    "id" | "opponent" | "ownerId" | "highlightNotificationSent"
  >,
): Promise<void> {
  if (game.highlightNotificationSent || game.ownerId == null) return;
  try {
    ...
    if (userRow?.pushToken) {
      await sendExpoPush(userRow.pushToken, {
        title: "Game highlights ready 🏀",
        body: `Your highlights vs ${game.opponent} are ready to watch`,
        data: { gameId: game.id },
